...
class MeatHook:
    ...
    api_sse_url = Template("https://api.particle.io/v1/.....")

    def __init__(self, device_id, token_id):
        self.device_id = device_id
        self.token_id = token_id
        self._subscribe_to_device()
    ...
    def _call_func(self, func_name, arg):
        try:
            r = requests.get(MeatHook.api_func_url.substitute(dict(device_id=self.device_id,
                                                                   func_name=func_name,
                                                                   arg=str(arg))),
                             params=dict(access_token=self.token_id))
            if r.status_code == requests.codes.ok:
    ...
    def _subscribe_to_device(self):
        events = SSEClient(self.api_sse_url)
        for event in events:
            # Figure out what the event is, and trigger the appropriate callback.
            # Probably just for alerts
            pass
